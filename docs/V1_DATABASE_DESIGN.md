...
> 说明：本文定义目标数据模型；Flyway V1~V3 已在本地标准开发库执行。
...
项目已引入 Flyway，并按只增不改的方式管理脚本：

| 版本 | 目标 |
| --- | --- |
| `V1__create_system_tables.sql` | 创建部门、用户、角色、权限及关联表 |
| `V2__create_ticket_tables.sql` | 创建分类、工单和工单记录表 |
| `V3__seed_reference_data.sql` | 初始化部门、分类、角色、权限及角色权限关系 |

开发演示账号尚未写入迁移文件；若后续需要，应使用独立的开发环境初始化机制，不能把弱密码账号写入正式迁移。
