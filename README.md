...
- 正式用户分页、详情、创建、资料修改、启停、角色替换和安全密码重置。
- 角色生命周期、角色权限替换与只读权限定义管理。
- Actuator 健康探针、生产环境敏感配置和真实依赖冒烟脚本。

V1.0 业务接口、数据库集成测试和 MySQL/Redis/Swagger 联调已完成；生产 profile 提供健康探针和安全默认值。
...
V1.0 收尾项已完成；后续可在该基线上按需求迭代通知、SLA 和统计报表等非目标能力。
